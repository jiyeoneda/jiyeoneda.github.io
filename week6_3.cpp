...
int main() {
	...
	for (int i = 0; i < n; i++) {
		...
		a = getAmplitude(ss);
		T = getLength(eu);
		for (int i = 0; i < T * sampleRate[0]; i++) {
			akbo[i] = (short)(a * sin(2 * pi * f1 * i * dt)+ a * sin(2 * pi * f2 * i * dt));
		}
		xx.write((char*)akbo, T * sampleRate[0] * sizeof(short));
	}
	song.close();
